...
async function verificarDocumento(input) {
    ...
    if (valor === "") {
        ...
        input.classList.remove('border-gray-300');
        // Mostra a mensagem
        document.getElementById('erro-docVazio').classList.remove('hidden');
        document.getElementById('erro-documento').classList.add('hidden');
        return null;
    }
    else if (!valido || valor === "00000000000" || valor === "00000000000000") {
        // Estilo de erro no Input
        input.classList.add('border-red-500', 'ring-red-500', 'bg-red-50');
        input.classList.remove('border-gray-300');
        // Mostra a mensagem
        document.getElementById('erro-documento').classList.remove('hidden');
        document.getElementById('erro-docVazio').classList.add('hidden');
        return null;
    } else {
        // Remove estilos de erro
        input.classList.remove('border-red-500', 'ring-red-500', 'bg-red-50');
...
async function verificarPassword(input) {
    ...
    if (valor === "") {
        ...
        input.classList.remove('border-gray-300');
        // Mostra a mensagem
        document.getElementById('erro-password').classList.remove('hidden');
        return null;
    } else {
        // Remove estilos de erro
        input.classList.remove('border-red-500', 'ring-red-500', 'bg-red-50');
        input.classList.add('border-gray-300');
        // Esconde a mensagem
        document.getElementById('erro-password').classList.add('hidden');
        return true;
...
document.getElementById('usuario').addEventListener('submit', async function (event) {
    ...
    let valorDoc = document.getElementById('documento');
    let documento = await verificarDocumento(valorDoc);
    ...
    let password = await verificarPassword(valorPassword);

    let valorConfirm = document.getElementById('password_confirm');
    let password_confirm = await verificarConfirm(valorConfirm);

    // Aqui você envia os dados usando fetch()
    try {
        if (!nome || !documento || !telefone || !email || !cep || !estado || !cidade || !bairro || !logradouro || !complementos || !password || !password_confirm) {
